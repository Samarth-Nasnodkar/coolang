...
std::pair<data, Error> list_add(data a, data b) {
  ...
  if (b._type == types::_list_type) {
    __runtime_var *_data = new __runtime_var[a.value._list->size + b.value._list->size];
    memcpy(_data, a.value._list->_data, a.value._list->size * sizeof(__runtime_var));
    memcpy(_data + a.value._list->size, b.value._list->_data, b.value._list->size * sizeof(__runtime_var));
    _res._list = new _llist{_data, a.value._list->size + b.value._list->size};
  } else {
    __runtime_var *_data = new __runtime_var[a.value._list->size + 1];
    memcpy(_data, a.value._list->_data, a.value._list->size * sizeof(__runtime_var));
    _data[a.value._list->size] = {__var, new data{b}};
...
std::pair<data, Error> list_mul(data a, data b) {
  ...
  if (b._type == types::_int) {
    ...
    __runtime_var *_data = new __runtime_var[final_len];
    for(int i = 0; i < b.value._int; ++i)
    memcpy(_data + (i * a_len), a.value._list->_data, a_len * sizeof(__runtime_var));
